...
MATCH_OVERRIDES: dict[str, dict[str, str]] = {
    ...
    "Batman vs Superman: Dawn of Justice": {"query": "Batman v Superman: Dawn of Justice"},
    ...
    "the league of extraordinary gentlemen": {"query": "The League of Extraordinary Gentlemen"},
        ...
        for row_match in SEARCH_ROW_PATTERN.finditer(body):
            ...
            candidates.append(
                SearchCandidate(
                    ...
                    cast=tuple(part.strip() for part in cast_text.split(",") if part.strip()),
                    tomatometer_score=int(tomatometer_text) if tomatometer_text else None,
                )
            )
    return candidates
...
def candidate_match_score(entry: MovieEntry, candidate: SearchCandidate, query: str) -> float:
    normalized_query = normalize_text(query)
    normalized_entry_title = normalize_text(entry.title)
    normalized_candidate_title = normalize_text(candidate.title)
    similarity = max(
        difflib.SequenceMatcher(None, normalized_query, normalized_candidate_title).ratio(),
        difflib.SequenceMatcher(None, normalized_entry_title, normalized_candidate_title).ratio(),
    )
...
def pick_best_candidate(
    ...
    if not candidates:
        raise ValueError(f"No Rotten Tomatoes search candidates found for {entry.title!r}")
    scored = [
        (candidate_match_score(entry, candidate, query), candidate) for candidate in candidates
    ]
...
def fetch_search_candidates(session: requests.Session, query: str) -> list[SearchCandidate]:
...
def build_ratings_dataframe(match_results: list[MatchResult]) -> pd.DataFrame:
    ...
    df["rt_average_rating"] = df[["rt_audience_rating", "rt_critic_rating"]].mean(axis=1)
...
def _regression_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> dict[str, float]:
    return {
        ...
        "correlation": (
            float(np.corrcoef(y_true, y_pred)[0, 1]) if len(y_true) > 1 else float("nan")
        ),
    }
...
def evaluate_models(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if len(set(y_train_like.tolist())) < 2:
        raise ValueError("Training split does not contain both liked and disliked examples")
    ...
    cutoff_holdout_pred = (holdout_df["rt_average_rating"].to_numpy(dtype=float) >= 60.0).astype(
        int
    )
    ...
    df["logistic_like_probability_full"] = full_logistic_model.predict_proba(full_X)[:, 1]
    ...
    return {
        ...
        "logistic_metrics": {
            ...
            "baseline_60_accuracy": float(accuracy_score(y_holdout_like, cutoff_holdout_pred)),
        },
...
def build_disagreement_tables(df_with_predictions: pd.DataFrame) -> dict[str, pd.DataFrame]:
...
def summarize_analysis(df: pd.DataFrame, model_results: dict[str, Any]) -> str:
    ...
    rule_60_holdout = float(((holdout["rt_average_rating"] >= 60.0) == holdout["liked"]).mean())
    lines = [
